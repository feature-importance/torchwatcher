...
To use a runningstats object,
    ...
    1. Create the desired stat object, e.g., `m = Mean()`
...
def tally(stat, dataset, cache=None, quiet=False, **kwargs):
    ...
    Subsampling is supported via sample_size= and random_sample=:
        ...
        used to select a fixed pseudorandom sample of size N to sample.
    """
...
class cache_load_enabled:
    ...
    tally from loading cached statistics, forcing them to be recomputed.
    """
...
class Bincount(Stat):
    ...
    def add(self, a, size=None, **kwargs):
        a = a.view(-1)
        bincount = a.bincount()
...
class Quantile(Stat):
    ...
    def median(self):
        return self.quantiles(torch.tensor([0.5]))
